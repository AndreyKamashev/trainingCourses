...
class Ujian extends CI_Controller {
	...
	public function __construct(){
		...
		$this->mhs 	= $this->ujian->getIdstudents($this->user->username);
    }

    public function akses_lecturer()
    {
        if ( !$this->ion_auth->in_group('Lecturer') ){
			show_error('Эта страница предназначена специально для преподавателей, которые могут провести онлайн-тест, <a href="'.base_url('dashboard').'">На главную</a>', 403, 'Forbidden Access');
		}
    }
	 public function akses_lecturer_admin()
    {
        if ( !($this->ion_auth->is_admin() || $this->ion_auth->in_group('Lecturer')) ){
			show_error('Эта страница предназначена специально для преподавателей и админов, которые могут провести онлайн-тест, <a href="'.base_url('dashboard').'">На главную</a>', 403, 'Forbidden Access');
		}
    }

    public function akses_students()
    {
        if ( !$this->ion_auth->in_group('Student') ){
			show_error('Эта страница специально предназначена для студентов, проходящих опрос. <a href="'.base_url('dashboard').'">На главную</a>', 403, 'Forbidden Access');
		}
    }
    ...
    public function master()
	{
		...
		if ($this->ion_auth->in_group('Lecturer')) {
			...
			$data = [
				'user' => $user,
				'judul'	=> 'Опросы',
				'subjudul'=> 'Данные опросов',
				'lecturer' => $this->ujian->getIdlecturer($user->username),
			];
			...
			$this->load->view('_templates/dashboard/_header.php', $data);
			$this->load->view('ujian/data_admin'); 
			$this->load->view('_templates/dashboard/_footer.php');
		}
	}
	...
	public function add()
	{
        ...
        $data = [
			'user' 		=> $user,
			'judul'		=> 'Опросы',
			'subjudul'	=> 'Добавить опрос',
			'courses'	=> $this->soal->getcourseslecturer($user->username),
			'lecturer'		=> $this->ujian->getIdlecturer($user->username),
		];
	...
	public function validasi()
	{
		...
		$user 	= $this->ion_auth->user()->row();
		$lecturer 	= $this->ujian->getIdlecturer($user->username);
		$jml 	= $this->ujian->getJumlahSoal($lecturer->id_lecturer)->jml_soal;
		$jml_a 	= $jml + 1; 

		$this->form_validation->set_rules('name_ujian', 'Наименование опроса', 'required|alpha_numeric_spaces|max_length[50]');
		$this->form_validation->set_rules('jumlah_soal', 'Количество вопросов', "required|integer|less_than[{$jml_a}]|greater_than[0]", ['less_than' => "Вопросов не достаточно: {$jml} "]);
		$this->form_validation->set_rules('tgl_mulai', 'Дата начала', 'required');
		$this->form_validation->set_rules('tgl_selesai', 'Дата завершения', 'required');
		$this->form_validation->set_rules('waktu', 'Время', 'required|integer|max_length[4]|greater_than[0]');
		$this->form_validation->set_rules('jenis', 'Сортировка вопросов', 'required|in_list[Random,Sort]');
	}
	...
	public function save()
	{
		...
		$name_ujian 	= $this->input->post('name_ujian', true);
		...
		if( $this->form_validation->run() === FALSE ){
			$data['status'] = false;
			$data['errors'] = [
				'name_ujian' 	=> form_error('name_ujian'),
	...
	public function list_json()
	{
		...
		$list = $this->ujian->getListUjian($this->mhs->id_students, $this->mhs->classes_id);
	...
	public function list()
	{
		...
		$data = [
			'user' 		=> $user,
			'judul'		=> 'Опросы',
			'subjudul'	=> 'Список опросов',
			'mhs' 		=> $this->ujian->getIdstudents($user->username),
		];
		$this->load->view('_templates/dashboard/_header.php', $data);
	...
	public function list_question()
	{
		...
		$data = [
			'user' 		=> $user,
			'judul'		=> 'Вопросы от преподавателя',
			'subjudul'	=> 'Список вопросов',		
		];
		$data['questions'] = $this->question->getDataQuestionSt($user->username);
		
		$this->load->view('_templates/dashboard/_header.php', $data);
		$this->load->view('ujian/list_question');
		$this->load->view('_templates/dashboard/_footer.php');
	}
	 //---------------edit--------------------
	public function editquest($id) {
		$this->form_validation->set_rules('name_answers','Ответ','required|trim');
		
		if($this->form_validation->run() === TRUE){ 
			//----------------------------------------
			$name_answers=$this->input->post('name_answers');
			$date_create = date('Y-m-d');
			$this->question->editquestSt($id,$name_answers,$date_create);
		} else {
			//-------------load edit-------------------
			  $user = $this->ion_auth->user()->row();
				$data = [
					'user' => $user,
					'judul'	=> 'Вопросы от преподавателя',
					'subjudul'=> 'Обновить ответ',
					'data' 		=> $this->question->getquestionById($id)
				];
							   
				$this->load->view('_templates/dashboard/_header.php', $data);
				$this->load->view('ujian/edit_question');
				$this->load->view('_templates/dashboard/_footer.php');		
		}
	}
	//--------------------------end text opros--------------------------------
	...
	public function index()
	{
		...
		$mhs		= $this->mhs;
		$h_ujian 	= $this->ujian->HslUjian($id, $mhs->id_students);
