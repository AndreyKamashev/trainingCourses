function load_themes() {
    $('#themes').find('option').not(':first').remove();

    $.getJSON(base_url+'themes/load_themes', function (data) {
        var option = [];
        for (let i = 0; i < data.length; i++) {
            option.push({
                id: data[i].id_themes,
                text: data[i].name_themes
            });
        }
        $('#themes').select2({
            data: option
...
function load_classes(id) {
    ...
    $.getJSON(base_url+'classes/classes_by_themes/' + id, function (data) {
        var option = [];
        for (let i = 0; i < data.length; i++) {
            option.push({
